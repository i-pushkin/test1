...
# Function to check if a port is open using netcat (nc)
function check_port() {
    local address=$1
    local port=$2
    nc -z -w5 $address $port
    return $?
}
...
# Check TFSS port accessibility using nc
address="1.1.1.1"
port=8501
...
if [ $port_status -eq 0 ]; then
    print_message 0 "Port $port on $address is accessible." "Port $port on $address is not accessible."
else
    print_message 1 "" "Port $port on $address is not accessible. Check failed."
fi

# Ports to check
ports_to_check=(8000 80 5555 9117 9187 9113)
...
if [[ $docker_status -eq 0 && $compose_status -eq 0 && $memory_status -eq 0 && $disk_status -eq 0 && $create_dirs_status -eq 0 && $dockerhub_status -eq 0 && $run_status -eq 0 && $port_status -eq 0 && $iptables_check_status -eq 0 ]]; then
